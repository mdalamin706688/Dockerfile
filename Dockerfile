...
# Start GeoServer
CMD ["sh", "-c", "cd /opt/geoserver/bin && ./startup.sh"]# Use OpenJDK 17 for better support and security
FROM openjdk:17-slim
...
# Install wget and tar (for bsdtar alternative)
RUN apt-get update && apt-get install -y wget tar && rm -rf /var/lib/apt/lists/*

# Download GeoServer
RUN wget -qO- https://sourceforge.net/projects/geoserver/files/GeoServer/$GEOSERVER_VERSION/geoserver-$GEOSERVER_VERSION-bin.zip/download -O geoserver.zip

# Extract GeoServer (using bsdtar by default)
RUN bsdtar -xvf geoserver.zip -C /opt

# Alternative extraction with tar (if bsdtar fails)
# RUN tar -xvf geoserver.zip -C /opt
